...
@pytest.mark.parametrize(
    "spec,p_rule",
    [
        (HybridStrategySpec("majority", n2=8, k_box=1, tie_bandwidth=0), 0.5),
        (HybridStrategySpec("horizontal", n2=4, k_box=3, split=2, order="maj_pyr", tie_bandwidth=0), 1.0),
    ],
    ids=lambda v: candidate_name(v) if isinstance(v, HybridStrategySpec) else str(v),
)
def test_exhaustive_k1_valid_probability_and_repeatable(spec: HybridStrategySpec, p_rule):
    strategy = _load_strategy_by_spec(spec)

    s1 = evaluate_strategy_exhaustive(strategy, p_rule=p_rule)
    s2 = evaluate_strategy_exhaustive(strategy, p_rule=p_rule)
